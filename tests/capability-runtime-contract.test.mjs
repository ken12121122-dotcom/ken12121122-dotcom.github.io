...
test('bridge inventory contains 40 capabilities and every built-in Node has Markdown', () => {
  ...
  assert.equal(1 + visible + (virtualPageTemplates - 1) + commandCount, 40);
  ...
  assert.equal(actualMd.length, 22);
  const bom = readFileSync(new URL('../android-native/docs/CAPABILITY_BOM.md', import.meta.url), 'utf8');
  assert.match(bom, /Existing Capability total \| 40/);
  assert.match(bom, /Chat-addressable \| 39/);
