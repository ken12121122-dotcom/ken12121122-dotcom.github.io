...
test('bridge inventory contains 44 capabilities and every built-in Node has Markdown', () => {
  ...
  assert.equal(1 + visible + (virtualPageTemplates - 1) + commandCount, 44);
  ...
  assert.equal(actualMd.length, 26);
  const bom = readFileSync(new URL('../android-native/docs/CAPABILITY_BOM.md', import.meta.url), 'utf8');
  assert.match(bom, /Existing Capability total \| 44/);
  assert.match(bom, /Chat-addressable \| 43/);
