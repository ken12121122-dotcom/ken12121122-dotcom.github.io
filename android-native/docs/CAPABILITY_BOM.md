...
| --- | ---: | --- |
| Existing Capability total | 40 | 23 Registry capabilities plus 17 existing commands |
| Chat-addressable | 39 | Fox can identify/read the Node context or the existing catalog can resolve the command |
...
| Managed Node MD assets | 22 | Every built-in functional Node; reference Nodes are excluded from the BOM |
...
| `app:fox-chat` — 狐狸 | `VoiceOrbHomeActivity`; `amin-fox://chat` | Implemented shared LLM chat entry | Direct, reads `fox-chat.md` | Future FoxPet connection remains OWNER-gated |
