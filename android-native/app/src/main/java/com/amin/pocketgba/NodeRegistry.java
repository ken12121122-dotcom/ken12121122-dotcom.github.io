...
final class NodeRegistry {
    ...
    static String navigationJson(Context context){
        ...
        try{
            ...
            if(infos!=null)for(ActivityInfo info:infos){
                Bundle m=info.metaData;
                if(m==null||!m.getBoolean(META_VISIBLE,false))continue;
                pages.put(pageFromMetadata(info,m));
            }
    ...
    static String typedEdgesJson(Context c,NodeMetadataStore o){
        ...
        try{
            ...
            JSONArray edges=root.optJSONArray("edges"); if(edges==null){edges=new JSONArray();root.put("edges",edges);}
            ...
            edges.put(GraphContract.edge("edge:finance:create:opens-food","app:finance-expense-create","app:finance-food","opens","active","1").put("authority","capability_projection"));
