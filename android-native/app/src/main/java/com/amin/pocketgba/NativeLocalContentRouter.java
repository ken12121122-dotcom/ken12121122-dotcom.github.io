...
final class NativeLocalContentRouter {
    private static final Pattern ROM_PATH = Pattern.compile(
            "^" + Pattern.quote(NativeCartridgeVaultBridge.ROM_PATH_PREFIX) + "([a-fA-F0-9]{64})\\.gba$"
    );
    ...
    WebResourceResponse intercept(WebResourceRequest request) {
        ...
        if (uri == null || !"https".equalsIgnoreCase(uri.getScheme()) || trustedHost == null || !trustedHost.equalsIgnoreCase(uri.getHost())) {
            return null;
        }
        String path = uri.getPath();
        if (path == null) return null;
        java.util.regex.Matcher romMatcher = ROM_PATH.matcher(path);
        if (romMatcher.matches()) return serveRom(romMatcher.group(1));
        if (path.startsWith(NativeCartridgeVaultBridge.ENGINE_PATH_PREFIX)) {
            return serveEngineAsset(path.substring(NativeCartridgeVaultBridge.ENGINE_PATH_PREFIX.length()));
        }
        return null;
    }
    ...
    private WebResourceResponse serveRom(String key) {
        ...
        try {
            ...
            return new WebResourceResponse("application/octet-stream", null, 200, "OK", headers, new FileInputStream(file));
    ...
    private WebResourceResponse serveEngineAsset(String relativePath) {
        ...
        if (safePath == null || safePath.isEmpty()) return textResponse(400, "Bad Request", "Invalid emulator asset path.");
        try {
            ...
            return new WebResourceResponse(mimeType(safePath), isTextAsset(safePath) ? "utf-8" : null, 200, "OK", headers, stream);
        } catch (Exception error) {
    ...
    private String sanitizeAssetPath(String value) {
        ...
        if (path.isEmpty() || path.contains("../") || path.equals("..") || path.indexOf('\0') >= 0) return null;
        return path;
    }
    ...
    private Map<String, String> baseHeaders(String cacheControl) {
        ...
        if (!trustedOrigin.isEmpty()) headers.put("Access-Control-Allow-Origin", trustedOrigin);
        return headers;
    }
    ...
    private WebResourceResponse textResponse(int status, String reason, String text) {
        ...
        return new WebResourceResponse("text/plain", "utf-8", status, reason, headers, new ByteArrayInputStream(bytes));
    }

    private boolean isTextAsset(String path) {
        String lower = path.toLowerCase(Locale.ROOT);
        return lower.endsWith(".js") || lower.endsWith(".css") || lower.endsWith(".json") || lower.endsWith(".html") || lower.endsWith(".txt") || lower.endsWith(".svg");
    }
