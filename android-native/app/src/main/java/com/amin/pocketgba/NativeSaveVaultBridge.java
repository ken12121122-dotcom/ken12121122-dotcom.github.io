...
public final class NativeSaveVaultBridge {
    ...
    public synchronized String putSave(String key, String base64Data) {
        ...
        try {
            ...
            if (base64Data == null || base64Data.length() > MAX_SAVE_BYTES * 2L) throw new SecurityException("Save payload exceeds safe size");
            byte[] bytes = Base64.decode(base64Data, Base64.DEFAULT);
            if (bytes.length == 0 || bytes.length > MAX_SAVE_BYTES) throw new SecurityException("Save byte length is invalid");
            ...
            if (current.isFile()) copyReplace(current, backup);
            moveReplace(temporary, current);
            long updatedAt = System.currentTimeMillis();
            if (!current.setLastModified(updatedAt)) updatedAt = current.lastModified();
            result.put("ok", true);
    ...
    public synchronized String getSave(String key) {
        try {
            byte[] bytes = readBestSave(requireKey(key));
    ...
    private String requireKey(String key) {
        ...
        if (!KEY_PATTERN.matcher(value).matches()) throw new SecurityException("Invalid save identity");
        return value.toLowerCase(Locale.ROOT);
    }

    private File currentFile(String key) { return new File(directory, key + ".sav"); }
    private File backupFile(String key) { return new File(directory, key + ".bak"); }
    private File temporaryFile(String key) { return new File(directory, key + ".tmp"); }
    ...
    private void writeFully(File target, byte[] bytes) throws Exception {
        try (FileOutputStream fileOutput = new FileOutputStream(target); BufferedOutputStream output = new BufferedOutputStream(fileOutput)) {
            output.write(bytes);
            output.flush();
    ...
    private byte[] readFully(File source) throws Exception {
        ...
        if (length <= 0 || length > MAX_SAVE_BYTES) throw new SecurityException("Stored save size is invalid");
        ...
        if (offset != bytes.length) throw new IllegalStateException("Stored save could not be read completely");
        return bytes;
    }
    ...
    private void deleteIfExists(File file) throws Exception {
        if (file.exists() && !file.delete()) throw new IllegalStateException("Unable to clear stale temporary save");
    }
