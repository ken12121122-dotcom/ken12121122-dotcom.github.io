...
public final class NativeCartridgeVaultBridge {
    ...
    @JavascriptInterface
    public synchronized String beginImport(String key, String displayName, long expectedSize) {
        JSONObject result = new JSONObject();
        try {
            String safeKey = requireKey(key);
    ...
    private String requireKey(String key) {
        ...
        if (!KEY_PATTERN.matcher(value).matches()) throw new SecurityException("Invalid ROM identity");
    ...
    private File romFile(String key) { return new File(directory, key + ".gba"); }
    private File metadataFile(String key) { return new File(directory, key + ".json"); }
    ...
    private void cancelImportsForKey(String key) {
        String[] ids = sessions.values().stream().filter(session -> session.key.equals(key)).map(session -> session.id).toArray(String[]::new);
    ...
    private static final class ImportSession {
        ...
        ImportSession(String id, String key, String displayName, long expectedSize, File temporary, FileOutputStream fileOutput, BufferedOutputStream output, MessageDigest digest) {
            this.id = id;
            this.key = key;
