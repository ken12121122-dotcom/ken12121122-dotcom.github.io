...
/** Generic data_view surface: list, add, single-record preview, refresh and filter context. */
public final class DataViewActivity extends Activity {
    ...
    private JSONObject node, filter=new JSONObject(), inputContext=new JSONObject();
    private DataContract contract;
    private StorageAdapter adapter;
    private TextView status, listView;
    ...
    private void resolve(){
        String nodeId=getIntent().getStringExtra(EXTRA_NODE_ID); node=NodeRegistry.findNode(this,metadataStore,nodeId);
        ...
        String contractId=node.optString("input_contract",getIntent().getStringExtra(SchemaFormActivity.EXTRA_CONTRACT_ID));
        JSONObject c=new ContractRegistry(this).get(contractId); if(c==null)c=ContractRegistry.transactionV1(); contract=new DataContract(c);
        JSONObject storage=node.optJSONObject("storage"); if(storage==null)storage=new JSONObject();
        String adapterId=storage.optString("adapter","google_sheets"),source=storage.optString("source_id","personal_finance"),table=storage.optString("table","Transactions");
        if("google_sheets".equals(adapterId))adapter=new GoogleSheetsAdapter(source,table,new GoogleSheetsApiTransport(this));
    }

    private void buildUi(){ScrollView scroll=new ScrollView(this);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(18),dp(22),dp(18),dp(30));scroll.addView(root,new ScrollView.LayoutParams(-1,-2));
        TextView title=text(node.optString("name",node.optString("title","Data View")),25);root.addView(title);
        status=text("Filter: "+filter.toString(),13);root.addView(status);
        ...
        listView=text("讀取中…",15);listView.setTextIsSelectable(true);root.addView(listView);setContentView(scroll);}
    ...
    @Override protected void onActivityResult(int request,int result,Intent data){super.onActivityResult(request,result,data);if(result!=RESULT_OK)return;if(request==REQ_FORM&&data!=null){String raw=data.getStringExtra(SchemaFormActivity.EXTRA_RESULT_JSON);append(parse(raw));}else if(request==REQ_SHEETS)refresh();}
    ...
    private void single(JSONObject r){if(r==null)return;new android.app.AlertDialog.Builder(this).setTitle("Record").setMessage(r.toString()).setPositiveButton("關閉",null).show();}
