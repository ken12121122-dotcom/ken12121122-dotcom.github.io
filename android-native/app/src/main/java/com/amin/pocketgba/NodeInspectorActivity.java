...
public final class NodeInspectorActivity extends Activity {
    ...
    private void buildUi(){
        ...
        Button md=button("Generate MD Preview");md.setOnClickListener(v->showMarkdown());root.addView(md);if(node.optJSONObject("input_context")!=null){Button context=button("查看關聯聊天 MD");context.setOnClickListener(v->showNodeContext());root.addView(context);}
    ...
    private void showNodeContext(){JSONObject context=NodeMdContextBuilder.build(this,store,nodeId);String content=context.optString("content","");if(content.isEmpty())content="無法讀取 MD。\n缺口："+String.valueOf(context.optJSONArray("unresolved_gaps"));TextView preview=label(content,14);preview.setTextIsSelectable(true);ScrollView wrap=new ScrollView(this);wrap.setPadding(dp(16),dp(8),dp(16),dp(8));wrap.addView(preview);String finalContent=content;new AlertDialog.Builder(this).setTitle("關聯聊天 MD · 唯讀").setView(wrap).setPositiveButton("Copy",(d,w)->copy(finalContent)).setNegativeButton("關閉",null).show();}
