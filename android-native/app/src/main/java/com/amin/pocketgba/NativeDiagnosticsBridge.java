...
public final class NativeDiagnosticsBridge {
    ...
    private static final Pattern DEVICE_PATH = Pattern.compile("(?:file:///)?/(?:data|storage|sdcard|mnt)/[^\\s)]+", Pattern.CASE_INSENSITIVE);
    private static final Pattern CONTENT_URI = Pattern.compile("content://[^\\s)]+", Pattern.CASE_INSENSITIVE);
    private static final Pattern EMAIL = Pattern.compile("[\\w.+-]+@[\\w.-]+\\.[A-Za-z]{2,}");
    private static final Pattern GAME_FILE = Pattern.compile("[^\\s/\\\\]+\\.(?:gba|bin|zip|sav|state)\\b", Pattern.CASE_INSENSITIVE);
    ...
    @JavascriptInterface
    public String getPendingReport() { return preferences.getString(PENDING_REPORT_KEY, ""); }
    ...
    @JavascriptInterface
    public String getEnvironment() { return buildEnvironment(appContext).toString(); }
    ...
    private static void installCrashHandler(Context context) {
        ...
        Thread.setDefaultUncaughtExceptionHandler((thread, throwable) -> {
            try {
                ...
                report.put("message", sanitize(throwable.getClass().getSimpleName() + ": " + String.valueOf(throwable.getMessage()), MAX_MESSAGE_CHARS));
    ...
    private static void savePending(Context context, JSONObject report) {
        context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).edit().putString(PENDING_REPORT_KEY, report.toString()).commit();
    }
    ...
    private static JSONObject buildEnvironment(Context context) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                if (webViewPackage != null) environment.put("webViewVersion", sanitize(webViewPackage.versionName, 80));
            }
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) environment.put("appVersionCode", appPackage.getLongVersionCode());
            else environment.put("appVersionCode", appPackage.versionCode);
        } catch (Exception ignored) {}
        return environment;
    }
