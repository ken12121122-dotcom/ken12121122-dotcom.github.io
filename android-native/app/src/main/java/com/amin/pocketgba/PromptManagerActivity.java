...
public final class PromptManagerActivity extends Activity {
    ...
    private void render(){
        ...
        for(PromptStore.Prompt prompt:rows){
            Button card=new Button(this); card.setTag(AminTheme.PRESERVE_TAG); card.setAllCaps(false); card.setGravity(Gravity.START|Gravity.CENTER_VERTICAL); card.setText(cardText(prompt)); card.setTextColor(p.text); card.setTextSize(15f); card.setBackgroundTintList(android.content.res.ColorStateList.valueOf(p.surface)); card.setMinHeight(dp(64)); card.setPadding(dp(12),dp(8),dp(12),dp(8));
    ...
    private String cardText(PromptStore.Prompt p){
        ...
        String auto=PromptText.preview(p.content.replace('\n',' '),40).trim();
        if(title.isEmpty() || title.equals(preview) || title.equals(auto) || preview.startsWith(title)) return prefix+preview;
        return prefix+title+"\n"+preview;
    }

    private Button chip(String s,boolean selected,AminTheme.Palette p){Button b=button(s,p);b.setTag(AminTheme.PRESERVE_TAG);b.setTextColor(selected?0xffffffff:p.primary);b.setBackgroundTintList(android.content.res.ColorStateList.valueOf(selected?p.primary:p.surfaceAlt));return b;}
