(() => {
  ...
  function parseNativeResult(raw, fallback = '原生卡匣庫作業失敗。') {
    let result;
    try { result = JSON.parse(raw || '{}'); } catch { throw new Error(fallback); }
  ...
  function recordHasRom(record) {
    return record?.blob instanceof Blob && record.blob.size > 0 && record.blob.size <= MAX_ROM_BYTES;
  }
  ...
  async function migrateRecord(record, { quiet = false } = {}) {
    ...
    if (native.hasRom(key)) return { key, url: native.getRomUrl(key), migrated: false, byteLength: record.blob.size };

    const started = parseNativeResult(native.beginImport(key, record.name || 'cartridge.gba', record.blob.size));
    ...
    try {
      for (let offset = 0; offset < record.blob.size; offset += CHUNK_BYTES) {
        ...
        parseNativeResult(native.appendImport(sessionId, bytesToBase64(bytes)));
        if (!quiet) setVaultStatus(`正在建立 Android 原生卡匣：${Math.round((end / record.blob.size) * 100)}%`, 'working');
        await sleep(0);
      }
      const finished = parseNativeResult(native.finishImport(sessionId));
      if (!finished.url || finished.sha256 !== key) throw new Error('Android 原生卡匣驗證結果不完整。');
      if (!quiet) setVaultStatus(`原生卡匣建立完成：${Math.ceil(finished.byteLength / 1024 / 1024)} MB · SHA-256 已驗證`, 'success');
    ...
    migrationPromise = (async () => {
      ...
      for (const record of records) {
        ...
        try {
          ...
          protectedCount += 1;
        } catch (error) { console.warn('[Native Cartridge Vault]', record?.id, error); }
      }
      setVaultStatus(protectedCount ? `Android 原生卡匣庫：${protectedCount} 款已保護${migrated ? `，本次搬移 ${migrated} 款` : ''}。` : '尚無可搬移的 ROM。', protectedCount ? 'success' : '');
  ...
  function numericGameId(value) {
    let hash = 2166136261;
    for (const char of String(value || 'amin-gba')) { hash ^= char.charCodeAt(0); hash = Math.imul(hash, 16777619); }
    return Math.abs(hash | 0) || 1;
  }

  function cleanName(value) {
    return String(value || 'GBA Game').split('/').pop().split('\\').pop().replace(/\.(gba|bin)$/i, '') || 'GBA Game';
  }
  ...
  function showPlayer(record) {
    ...
    if ($('activeGameTitle')) $('activeGameTitle').textContent = `正在從 Android 卡匣庫啟動 ${cleanName(record.name)}…`;
  ...
  function dispatchLaunch(status, detail = {}) {
    const payload = { status, at: Date.now(), nativeVaultV2: true, ...detail };
  ...
  async function reportFailure(record, message, stage = 'native-vault-v2') {
    dispatchLaunch('failed', { stage, romId: record?.id || null, message, transport: 'android-private-file', engine: 'apk-bundled-emulatorjs-4.2.3', core: 'gba/mgba' });
    try {
      await window.AMIN_ERROR_REPORTER?.enqueue?.({ category: 'emulator-launch', stage, message, environment: { emulatorChannel: 'apk-bundled-4.2.3', romHashPrefix: /^[a-f0-9]{8,64}$/i.test(record?.id || '') ? String(record.id).slice(0, 12) : null, lastSuccessfulStage: stage } });
      await window.AMIN_ERROR_REPORTER?.sendQueue?.();
    } catch {}
  }

  function emulatorMessage() {
    return String(window.EJS_emulator?.textElem?.innerText || $('game')?.querySelector('.ejs_error_text')?.innerText || $('game')?.innerText || '').trim();
  }
  ...
  async function waitForStart(state) {
    ...
    while (Date.now() - startedAt < START_TIMEOUT_MS) {
      ...
      if (window.EJS_emulator?.failedToStart) throw new Error(emulatorMessage() || '內建 mGBA 核心回報啟動失敗。');
      await sleep(200);
    }
  ...
  async function launch(id) {
    ...
    try {
      ...
      if (!recordHasRom(record)) throw new Error('舊遊戲庫已沒有完整 ROM，請重新匯入合法卡帶備份一次。');
      setStatus('建立原生卡匣…');
      ...
      window.AMIN_GBA_SAVE_GUARD?.setGame?.({ saveKey, romId: record.id || cartridge.key, romName: record.name, stableName });
      showPlayer(record);
      setStatus('啟動本地 mGBA…');
      ...
      window.EJS_ready = () => dispatchLaunch('emulator-ready', { romId: record.id, engineBase, transport: 'android-private-file' });
      window.EJS_onGameStart = () => {
        state.started = true;
        window.__AMIN_GBA_CORE_READY__ = true;
        $('gameLoading')?.classList.add('hidden');
        setTimeout(() => window.AMIN_GBA_SAVE_GUARD?.restore?.().catch?.(() => {}), 900);
      };
      window.EJS_onExit = () => {
        window.__AMIN_GBA_CORE_READY__ = false;
      ...
      dispatchLaunch('starting', { romId: record.id, cartridgeKey: cartridge.key, romUrl, engineBase, transport: 'android-private-file', engine: 'apk-bundled-emulatorjs-4.2.3', core: 'gba/mgba' });

      loaderScript = document.createElement('script');
      loaderScript.id = 'amin-native-vault-v2-loader';
      loaderScript.src = `${engineBase}loader.js`;
      loaderScript.async = true;
      loaderScript.onerror = () => { state.loaderError = 'APK 內建 EmulatorJS loader 無法讀取。'; };
      document.body.appendChild(loaderScript);
      await waitForStart(state);
      $('gameLoading')?.classList.add('hidden');
      setStatus('本地 mGBA 運作中', 'active');
      setImportStatus(`啟動成功：ROM ${Math.ceil(cartridge.byteLength / 1024 / 1024)} MB 已由 Android 私有卡匣庫供應。`, 'success');
      dispatchLaunch('started', { romId: record.id, cartridgeKey: cartridge.key, transport: 'android-private-file' });
      return true;
    } catch (error) {
  ...
  addEventListener('amin-native-cartridge-imported', () => setTimeout(() => migrateLibrary().catch(() => {}), 1200));

  window.AMIN_NATIVE_CARTRIDGE_VAULT_V2 = { version: '2.0.0', available: enabled, migrateRecord, migrateLibrary, launch };
  if (enabled()) {
    document.documentElement.dataset.nativeCartridgeVault = 'v2';
    setTimeout(() => migrateLibrary().catch(error => setVaultStatus(error?.message || '原生卡匣庫搬移失敗。', 'error')), 1100);
  }
})();
