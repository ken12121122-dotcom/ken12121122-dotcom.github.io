...
| v0.9.0 交接檔 | [`/AMIN_POCKET_GBA_HANDOFF.md`](../AMIN_POCKET_GBA_HANDOFF.md) | 2026-07-12 | 手把訊號診斷起點、PWA→原生橋接的轉折、v0.9.0 當時的已驗證/未驗證分界表。**已過時**：現況已到 Bridge 23，本檔案許多「未驗證」項目後續文件顯示已完成，見 [`known-issues.md`](./known-issues.md)。 |
...
| （外部）Dify repo | [`github.com/langgenius/dify`](https://github.com/langgenius/dify) | 讀取當下的 default branch | LLM 應用開發平台。收錄目的是研究它的節點-連結（node-link）workflow 架構，見 [`reference-dify-node-architecture.md`](./reference-dify-node-architecture.md)。跟 Amin 專案目前**沒有**已確認的關聯，純參考資料。 |
...
| 永久簽章身分紀錄 | [`/android-native/PERMANENT_SIGNING_IDENTITY.md`](../android-native/PERMANENT_SIGNING_IDENTITY.md) | 同上 | 只記錄公開憑證指紋，聲稱 Bridge 1（code 97）起的簽章身分。**⚠️ 其記錄的指紋跟其他所有來源不一致**，見 [`release-process.md`](./release-process.md) 的簽章矛盾段落。 |
...
| （對話口述）使用者個人背景 | 無檔案，使用者對話口述 | 2026-08-06 | 使用者自我介紹（嘉義人、志願役空軍 5 年含消防兵與氣象室、之後歷任建築師助理／生產線儲備幹部／倉庫管理員／現職職安主管）。**跟 Amin Pocket GBA 專案本身無關**，使用者要求仍收錄進本 wiki，見 [`user-profile.md`](./user-profile.md)（獨立成頁，不連進任何專案頁面）。 |

## 已知：來源之間的日期落差

`AGENTS.md` 與 `ARCHITECTURE.md` 都標註「最後人工驗證：2026-07-19」且明確寫「正式版本是 Bridge 16」，但 `native-release-manifest.json` 的 `publishedAt` 是 `2026-07-22`、`latestVersionName` 已經是 `bridge23`，git log 也顯示 Bridge 17 到 Bridge 23 都已經有提交。這代表 `AGENTS.md`／`ARCHITECTURE.md` 沒有跟上最新幾次發布 —— 細節與影響見 [`release-process.md`](./release-process.md) 的「⚠️ 矛盾」段落。
